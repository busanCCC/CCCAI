...
const GUEST_NOTICE_MESSAGE = "로그인을 하면 씨앗 순장님이 더 좋은 답변을 제공해줘요.";
...
export function ChatShell() {
  ...
  }, [cancelConversationLoad, hydrateSession, isAuthLoading, refreshInputSuggestions, user?.id]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (currentState.userId !== authUserId || currentState.conversationId !== conversationId) {
          return;
        }
  ...
  const handleSelectConversation = useCallback(
    async (selectedConversationId: string) => {
      ...
      try {
        ...
        if (conversationLoadRequestIdRef.current !== requestId) return;
        const message = e instanceof Error ? e.message : "대화를 불러오지 못했습니다.";
        toast.error(message);
      } finally {
        finishConversationLoad(requestId);
      }
          ...
          <div className="mx-auto max-w-[600px] space-y-1.5">
            ...
            {!user && messages.length === 0 && (
              <div className="flex justify-end">
                <div className="animate-fade-in relative mr-[72px] max-w-[240px]">
                  <div className="absolute -top-1 right-6 h-3 w-3 rotate-45 rounded-[2px] border-t border-l border-[#dec47a] bg-[#fff2bf]" />
                  <div className="rounded-[18px] border border-[#dec47a] bg-[linear-gradient(135deg,#fff9e4_0%,#ffefb6_100%)] px-3.5 py-2 text-[11px] leading-[1.45] font-semibold tracking-[-0.01em] text-[#6d5314] shadow-[0_10px_24px_rgba(171,136,44,0.16)] sm:text-xs">
                    {GUEST_NOTICE_MESSAGE}
                  </div>
                </div>
              </div>
            )}
